...
export default function AddWidgetDialog({
  ...
  const toggleWidgetInCategory = useDashboardStore(
    (s) => s.toggleWidgetInCategory,
  );
  ...
  function createWidget() {
    if (!name.trim()) return;
    addWidgetToCategory(categoryId, {
      name: name.trim(),
      text: text.trim() || "New widget",
    });
    setName("");
    setText("");
    setOpen(false);
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <Tabs defaultValue="create">
          ...
          <TabsContent value="create" className="space-y-3 pt-3">
            ...
            />
            <DialogFooter>
              <Button onClick={createWidget} className="ml-auto">
                Add
              </Button>
            </DialogFooter>
          </TabsContent>
            ...
            <div className="max-h-72 overflow-auto pr-2 space-y-2">
              {filtered.map((w) => {
                ...
                return (
                  <label
                    key={w.id}
                    className="flex items-start gap-3 px-2 py-2 rounded-md hover:bg-muted cursor-pointer"
                  >
                    <Checkbox
                      checked={checked}
                      onCheckedChange={(v) =>
                        toggleWidgetInCategory(categoryId, w.id, Boolean(v))
                      }
                    />
                    <div>
