...
export interface DashboardState {
  ...
  // mutations
  addWidgetToCategory: (categoryId: string, widget: { name: string; text: string; id?: string }) => string;
...
function normalize() {
  ...
  for (const c of data.categories) categories[c.id] = { ...c, widgetIds: [...c.widgetIds] };
    ...
    (set, get) => {
      ...
      return {
        ...
        getAvailableWidgetsForCategory: (id) => {
          const cat = get().categories[id];
          if (!cat) return [];
          return Object.values(get().widgets).filter((w) => !cat.widgetIds.includes(w.id));
        },
        addWidgetToCategory: (categoryId, widget) => {
          ...
          set((state) => {
            ...
            return { ...state, widgets: newWidgets, categories: { ...state.categories, [categoryId]: newCat } };
          });
          return id;
        },
        removeWidgetFromCategory: (categoryId, widgetId) => {
          set((state) => {
            ...
            return { ...state, categories: { ...state.categories, [categoryId]: newCat } };
          });
        },
        toggleWidgetInCategory: (categoryId, widgetId, checked) => {
          if (checked) get().addWidgetToCategory(categoryId, { id: widgetId, name: get().widgets[widgetId]?.name ?? widgetId, text: get().widgets[widgetId]?.text ?? "" });
